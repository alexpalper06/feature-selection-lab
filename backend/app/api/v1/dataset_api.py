...
# established prefix. For example, if one route is /, the real URL is "/datasets", if its /analyze, it'd be "/datasets/analyze"
...
async def upload_dataset(
    ...
    """
    Second part of uploading the dataset. Confirms upload and persists to database.
    """
    ...
    return await dataset_service.create_dataset(session, form_data)


@router.get("/", response_model=List[DatasetRead])
def list_datasets(session: Session = Depends(get_session)):
    """Returns all uploaded datasets ordered by creation date."""
...
async def get_dataset(dataset_id: int, session: Session = Depends(get_session)):
    """Returns database metadata and some entries for a single dataset."""
    return await dataset_service.get_dataset_details(session, dataset_id)


@router.patch("/{dataset_id}", response_model=DatasetRead)
def rename_dataset(dataset_id: int, body: DatasetUpdate, session: Session = Depends(get_session)):
