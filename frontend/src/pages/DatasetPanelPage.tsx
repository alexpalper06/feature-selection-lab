...
export default function DatasetPanelPage(): React.ReactElement {
    ...
    if (error || !dataset) {
        return (
            <AppLayout breadcrumbs={[{label: 'Datasets', href: '/datasets'}]}>
                <div
                    className="p-4 bg-red-500/10 border border-red-500/20 text-red-500 rounded-xl flex items-center gap-3 text-sm max-w-4xl mx-auto mt-6">
            ...
            <div className="w-full flex flex-col min-w-0 overflow-x-hidden">
                ...
                <div className="w-full flex-1 px-4 sm:px-6 lg:px-8 py-6">
                    <div className="max-w-7xl mx-auto w-full">
                        ...
                        {activeTab === 'configure' && (
                            <ConfigurationTab dataset={dataset}/>
                        )}
                        {activeTab === 'history' &&
                            <HistoryTab datasetId={dataset.id} targetVariables={dataset.target_variables}/>}
